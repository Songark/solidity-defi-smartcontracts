...
contract CSAMM {
    ...
    function getSwapRate(address inToken, uint inAmount) public view returns (uint) {
        if (inToken == address(token0)) {
            return (token0.balanceOf(address(this)) + inAmount - reserve0) * 997 / 1000;
        } else if (inToken == address(token1)) {
            return (token1.balanceOf(address(this)) + inAmount - reserve1) * 997 / 1000;
        } else {
            revert ("Input token is not valid.");
        }
    }
}
