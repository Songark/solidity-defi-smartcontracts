...
describe("AMM_CSAMM", function () {
  // We define a fixture to reuse the same setup in every test.
  // We use loadFixture to run this setup once, snapshot that state,
  // and reset Hardhat Network to that snapshot in every test.
  async function deployCSAMM() {
    ...
    const csamm = await CSAMM.deploy(token1.target, token2.target);
    ...
    return { csamm, token1, token2, owner, account1, account2 };
  }

  describe("Constant Sum AMM", function () {
    let contract;
    ...
    before(async () => {
      const deployedInfo = await loadFixture(deployCSAMM);
      contract = deployedInfo.csamm;
